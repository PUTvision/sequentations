...
from sequentations.core.composition import Compose
from sequentations.pytorch.transforms import ToTensorV2
from sequentations.augmentations.transforms import ColorJitter, Flip, Normalize, RandomGamma, Rotate, Resize
...
def test_compose_keypoints():
    _transforms = Compose([
        Flip(always_apply=True),
        Resize(200,200, always_apply=True)
    ],
    keypoint_params=A.KeypointParams(format='xy'))

    frame = np.array(np.random.rand(2, 400, 400, 3) * 255, dtype=np.uint8)
    keypoints = np.array(
        [
            (200, 398)
        ])

    transformed = _transforms(image=frame, keypoints=keypoints)
    frame, keypoints = transformed['image'], transformed['keypoints']

    assert keypoints == [(99.5, 199.0)] or [(100.0, 0.5)]
